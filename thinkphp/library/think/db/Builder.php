    ...
    public function buildWhere(Query $query, $where)
    {
        ...
        foreach ($where as $logic => $val) {
            ...
            foreach ($val as $value) {
                if (is_array($value)) {
                    if (key($value) !== 0) {
                        throw new Exception('where express errorInfo:' . var_export($value, true));
                    }
    ...
    protected function parseWhereItem(Query $query, $field, $val, $rule = '', $binds = [], $bindName = null)
    {
        ...
        if (!in_array($exp, $this->exp)) {
            ...
            if (isset($this->exp[$exp])) {
                ...
                throw new Exception('where express errorInfo:' . $exp);
            }
        }
        ...
        if (in_array($exp, ['=', '<>', '>', '>=', '<', '<='])) {
            if (is_array($value)) {
                throw new Exception('where express errorInfo:' . $exp . var_export($value, true));
            }
