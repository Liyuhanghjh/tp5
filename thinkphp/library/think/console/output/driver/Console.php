...
class Console
{
    ...
    public function renderException(\Exception $e)
    {
        ...
        do {
            ...
            $messages   = ['', ''];
            $messages[] = $emptyLine = sprintf('<errorInfo>%s</errorInfo>', str_repeat(' ', $len));
            $messages[] = sprintf('<errorInfo>%s%s</errorInfo>', $title, str_repeat(' ', max(0, $len - $this->stringWidth($title))));
            foreach ($lines as $line) {
                $messages[] = sprintf('<errorInfo>  %s  %s</errorInfo>', $line[0], str_repeat(' ', $len - $line[1]));
            }
            $messages[] = $emptyLine;
            $messages[] = '';
            $messages[] = '';
